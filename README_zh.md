...
### 自我进化
每次任务完成后，Agent 会分析自己的执行过程：
- **学习技能**: 同类任务成功 3 次以上 → 自动生成 SKILL.md → 下次复用
- **持续优化**: GEPA 进化引擎持续优化 prompt → 准确率↑，token 成本↓
- **记住偏好**: 学习你偏好的语言、框架、沟通风格
- **预测需求**: 检测日常模式（如每天 9 点部署） → 主动准备检查清单
...
| 层级 | 功能 | 示例 |
|------|------|------|
| L1 冻结 | 长期记忆，持久化到文件 | "用户偏好 Python，不喜欢 Java" |
| L2 程序性 | 自动学习的可复用工作流 | "部署 Django 的 7 个标准步骤" |
| L3 FTS5 索引 | 对话历史全文搜索 | "上周讨论过的数据库迁移方案" |
| L4 预测 | **预判你的下一步** | "你每次改完代码都会运行测试。现在运行吗？" |

第 4 层是 DeepSoul 的创新：从被动回忆到主动预测。
