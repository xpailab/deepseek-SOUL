...
class PromptBuilder:
    ...
    def build_system_prompt(
        ...
        # === 静态层（prefix cache 友好——各请求间不变） ===
        # 1. 静态身份文件（SOUL、IDENTITY、AGENTS、TOOLS）
        for filename, section_name, required in PROMPT_FILES:
            if filename in ("MEMORY.md", "USER.md"):
                continue  # 动态文件排到后面
        ...
        # === 动态层（每次请求可能变化——排最后，不影响 prefix cache） ===
        # 6. 动态身份文件（MEMORY.md、USER.md）
        for filename in ("MEMORY.md", "USER.md"):
            content = self._read_prompt_file(filename)
            if content:
                content = self._sanitize(content)
                for _, section_name, _ in PROMPT_FILES:
                    if _ == filename:
                        sections.append(f"<{section_name}>\n{content}\n</{section_name}>")
                        break
...
- 主动检测和防御 prompt injection 攻击
    ...
    def _global_rules(self) -> str:
        """生成全局行为规则——精简版，始终需要的规则。首轮专用规则在 _first_injection() 中。"""
...
- 将技术输出整理成易读的格式
    ...
    @staticmethod
    def _first_round_injection() -> str:
        """首轮专用指令——只在每条新用户消息的第一轮注入，后续工具轮不需要。"""
        return """<first_round_instructions>
## 侦察阶段规则（关键——动手前先看清楚）
...
- 侦察完成后，在回复中总结发现，然后基于侦察结果制定执行计划，而不是基于猜测
- 简单问候、纯知识问答可以跳过侦察

## 模糊任务反问规则（关键）
- 当用户的任务描述过于简单或模糊时，你必须先反问 1-2 个关键问题
- 判断标准: 如果任务描述少于 25 个字且不涉及具体文件/路径/错误信息，则视为模糊
